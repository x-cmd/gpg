...
- **公钥仓库**：发布方维护的数据路径（公开仓库或专用
  CDN），把所有历年年度公钥（`key-2025.gpg`、
  `key-2026.gpg` …）合并为单个 keyring，让消费者一次拉
  取就拥有所有选项。
- **用户管理的密钥环**：消费者把 keyring 导入自己的
  `gpg`，于是在本地同时持有历史与未来密钥。从此供应链归
  用户：用户的 `gpg` 密钥环是用户自己的，"始终保留旧密
  钥 / 跨年时删除 / 自定节奏轮换"的策略由用户定。发布方
  的唯一职责是发字节；用户的密钥环、用户的供应链策略。