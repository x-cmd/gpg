...
## 团队发布的两把密钥（FAQ Q4）—— *候选设计*

团队正在考虑的供应链密钥方案会发布两把签名密钥，各有不
同运营定位。**截至本文撰写时，`index.tsv` 中并无此类
行** —— 本节描述的是 *候选* 设计，而非现状。

- **社区版密钥** —— 将用于签名标准社区包（`x-cmd.rpm` /
  `x-cmd.deb`）。一次导入，未来升级静默校验。
- **年度密钥（`key-<year>`）** —— 将用于签名年度合规包
  （`x-cmd-annual-<year>.rpm`）。严格的逐年隔离，满足
  金融 / 政企采购审计。

按此候选设计，两把密钥都将位于本仓库 `keyring/` 下，
