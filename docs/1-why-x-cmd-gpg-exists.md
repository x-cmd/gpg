...
(the keypair, which holds the cryptographic material). GPG
without keys has nothing to encrypt with; keys without GPG
have nothing that can perform the operations. The two are
inseparable in practice. See article 0 for the full answer.

### Q2: Sigstore exists now — why do RPM / DEB still rely on GPG?

The answer is historical compatibility with the OS-native
toolchain. RPM (`dnf` / `yum`) and DEB (`apt`) verification
engines were designed with deep OpenPGP integration from day
one and remain 100% dependent on GPG keys for package
signing; Sigstore is widely adopted in cloud-native contexts
(Docker, Kubernetes, npm / PyPI) but does not replace GPG in
the OS-package channel. The two ecosystems co-exist; for
OS-level packages the de-facto channel is still GPG. See
article 0 for the full answer.

### Q3: Why don't publishers usually distribute unsigned raw packages?

Two main disadvantages tip the balance against unsigned
distribution: network transmission and CDN nodes lack
cryptographic tamper protection (MITM and package poisoning
become trivial), and most Linux package managers pop up an
error and refuse to install unsigned packages by default.
The advantages (zero key-management overhead, offline
re-signing) don't outweigh those in practice. See article 0
for the full answer.