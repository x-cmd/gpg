...
是软件程序与数据凭证的关系。GPG 是执行加密、解密、签名
生成与校验等 *计算动作* 的程序；GPG Key 是程序所需的密
码学材料（公钥半可公开、私钥半必须保密）。两者在实践中
不可分割 —— GPG 无密钥无可用材料，密钥无 GPG 无可执行
的操作。完整答案见文章 0。

### Q2：既然有了 Sigstore（无密钥签名），为什么 RPM / DEB 依然高度依赖 GPG？

答案是与操作系统原生工具链的历史兼容性。RPM（`dnf` /
`yum`）与 DEB（`apt`）的校验引擎自设计之初就深度绑定
OpenPGP（GPG），至今 100% 依赖 GPG 密钥签署包或源索引
文件；Sigstore 在云原生（Docker、Kubernetes、npm /
PyPI）领域广泛采用，但不替代 OS 包管理中的 GPG 渠道。
两者并存；OS 级包分发的事实渠道仍是 GPG。完整答案见
文章 0。

### Q3：为什么发布者通常不直接分发未签名的裸包？

两点劣势压倒了优势：网络传输与 CDN 节点缺乏密码学防篡
改保护，MITM 与包投毒变得轻而易举；多数现代 Linux 发行
版包管理器默认弹窗报错并拒绝安装未签名包。零密钥管理开
销、离线重签等优势在实践中无法弥补这两点。完整答案见文
章 0。