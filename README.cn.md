...
当前设计：在 GitHub Release 中同时提供两种已签名的发布形
态。这两种形态分开存在，是因为各自面对的用户需求不同 ——
硬塞进同一把密钥会让一方被迫接受另一方的权衡。

1. **`x-cmd.rpm` / `x-cmd.deb`（社区版）**：使用团队主密钥
   签署。面向开源极客与普通容器构建场景。一次导入，后续
   升级静默校验。
2. **`x-cmd-annual-<年份>.rpm` / `.deb`（年度版）**：使用
   当年专属的年度隔离密钥（如 `key-2026`）签署。面向需要
   在年度审计里做严格隔离的客户（常见：金融、政企采购）。
...
制。重签后的老包公开托管，与原签版本并列。

操作要点：RPM 重新签名不需要重新编译老代码，也不修改上
游 GitHub Release 资产。CI/CD 中只需通过 `rpmsign --addsign`
追加一道签名 Header 即可。

服务分级：哪些历史版本获得重签、多久重签一次，由团队根
据实际请求与维护成本决定。免费用户使用 `keyring/archive/`
中的历史密钥自行校验历史制品 —— 这是 GPG 设计的默认能
力。需要团队代为完成重签的用户，可通过支持渠道提出具体
请求；处理速度与深度取决于双方维护关系的具体约定。
